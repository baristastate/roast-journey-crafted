...
export const Route = createFileRoute("/heimroester")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Mit unseren Heimröstern steuerst du Frische, Röstgrad und Geschmack zuhause. Einfach, kontrolliert, emotional." },
...
const ROAST_PROFILES = [
  { id: "hell", label: "Hell", color: "oklch(0.55 0.09 60)", aroma: "Fruchtig, klar, lebendig.", profile: "Höhere Temperatur, kurze Entwicklung. Säure bleibt erhalten." },
  { id: "mittel", label: "Mittel", color: "oklch(0.38 0.07 45)", aroma: "Ausgewogen, süß, rund.", profile: "Mittlere Entwicklung. Süße und Körper im Vordergrund." },
  { id: "dunkel", label: "Dunkel", color: "oklch(0.22 0.04 35)", aroma: "Kräftig, schokoladig, intensiv.", profile: "Längere Röstung. Tiefe Aromen, weniger Säure." },
] as const;
  ...
  return (
    <section className="theme-dark relative bg-espresso text-cream overflow-hidden min-h-[100svh] flex items-center pt-24">
      <div className="absolute inset-0 -z-10">
        <img src={heroImg} alt="Heimröster im Lichtkegel" className="h-full w-full object-cover opacity-70" />
      ...
      <div className="mx-auto max-w-[1400px] px-5 md:px-10 py-20 grid lg:grid-cols-12 gap-10 items-center">
        <div className="lg:col-span-7">
          ...
          <motion.h1
            ...
            className="mt-5 font-display tracking-display text-5xl md:text-7xl lg:text-8xl leading-[0.95]"
          >
            Röste deinen Kaffee selbst.<br />
            <span className="italic font-light text-amber">Einfacher,</span> als du denkst.
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.3 }} className="mt-7 max-w-xl text-cream/75 text-lg">
            Mit unseren Heimröstern steuerst du Frische, Röstgrad und Geschmack direkt zuhause.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.5 }} className="mt-10 flex flex-wrap gap-3">
            <a href="#sortiment" className="rounded-full bg-amber px-6 py-3.5 text-sm font-medium text-espresso">Heimröster entdecken</a>
            <a href="#so-gehts" className="rounded-full border border-cream/30 px-6 py-3.5 text-sm">So funktioniert es</a>
          </motion.div>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} className="mt-12 text-sm text-cream/60 max-w-md">
...
function RoastSequence() {
  ...
  return (
    <section id="so-gehts" ref={ref} className="theme-dark bg-espresso text-cream" style={{ height: "260svh" }}>
        ...
        <div className="mx-auto max-w-[1400px] px-5 md:px-10 grid lg:grid-cols-2 gap-12 items-center w-full">
          ...
          <div>
            ...
            <h2 className="mt-4 font-display tracking-display text-4xl md:text-6xl leading-[1]">
              Aus Rohkaffee wird dein<br /><span className="italic text-amber font-light">persönlicher Kaffee.</span>
            </h2>
            <p className="mt-6 text-cream/70 max-w-md">
              Beim Scrollen folgst du der Bohne durch jede Phase der Röstung — von hellgrün bis tief schokoladig.
            </p>
...
function RoastBeans({ idx, stages }: { idx: any; stages: { c: string; label: string }[] }) {
  const color = useTransform(idx, stages.map((_, i) => i), stages.map((s) => s.c));
  const wobble = useTransform(idx, (v: number) => Math.sin(v * 6) * 4);
  const beans = Array.from({ length: 22 });
  return (
    <motion.div className="relative h-[420px] w-[420px] max-w-full rounded-full" style={{ background: "radial-gradient(circle at 50% 40%, rgba(232,180,90,0.15), transparent 65%)" }}>
      {beans.map((_, i) => {
        ...
        return (
          <motion.span
            ...
            transition={{ duration: 3 + (i % 4) * 0.5, repeat: Infinity, ease: "easeInOut", delay: i * 0.08 }}
          />
        );
      })}
      <motion.div style={{ background: color }} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-12 w-20 rounded-[40%] shadow-2xl" />
    </motion.div>
  );
}

function StageList({ idx, stages }: { idx: any; stages: { c: string; label: string }[] }) {
  return (
...
function StageRow({ idx, target, label, color }: { idx: any; target: number; label: string; color: string }) {
      ...
      <div className="mx-auto max-w-[1400px] px-5 md:px-10 grid lg:grid-cols-2 gap-12 items-center">
        <Reveal>
          ...
          <h2 className="mt-4 font-display tracking-display text-5xl md:text-6xl leading-[1]">
            Der Moment, in dem<br /><span className="italic text-amber font-light">Kaffee entsteht.</span>
          </h2>
          <p className="mt-6 text-cream/70 max-w-md">Du hörst ihn. Ein leises, klares Knacken. Die Bohne bricht auf — Zucker karamellisiert, Aromen entfalten sich. Ab hier bestimmst du, wohin der Kaffee geht.</p>
        </Reveal>
        <Reveal delay={0.1}>
          <TempCurve />
        </Reveal>
...
function TempCurve() {
  return (
    <div className="relative aspect-[5/3] rounded-3xl border border-cream/15 p-6 bg-espresso">
      <div className="text-[0.7rem] uppercase tracking-[0.24em] text-cream/50 mb-2">Bohnen-Temperatur · °C</div>
      <svg viewBox="0 0 500 250" className="w-full h-[calc(100%-2rem)]">
        ...
        <path d="M 0 230 C 80 200 140 170 220 130 S 360 60 500 30" fill="none" stroke="url(#curve)" strokeWidth="3" strokeLinecap="round" strokeDasharray="800" strokeDashoffset="0">
        ...
        <g>
          ...
          <text x="332" y="86" fill="var(--cream)" fontSize="12" className="font-medium">First Crack · ~196°C</text>
        </g>
      </svg>
    </div>
  );
        ...
        <div className="grid lg:grid-cols-12 gap-10 items-center">
          ...
          <div className="lg:col-span-7">
            <div className="rounded-3xl border border-border bg-card p-8 md:p-10">
              ...
              <div className="px-2">
                <input
                  type="range" min={0} max={2} step={1} value={i} onChange={(e) => setI(Number(e.target.value))}
                  className="w-full accent-amber"
                />
                <div className="mt-3 flex justify-between text-xs uppercase tracking-[0.2em] text-muted-foreground">
                  {ROAST_PROFILES.map((r) => <span key={r.id}>{r.label}</span>)}
                </div>
              </div>
              <motion.div key={`txt-${p.id}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-8 border-t border-border pt-6">
                <h3 className="font-display text-2xl">{p.label} · {p.aroma}</h3>
...
function Hotspots() {
  const items = [
    ...
    ["04", "Frische selbst bestimmen", "Röste, wenn du brauchst — nicht, wenn ein Lager es zulässt."],
    ...
    <section id="sortiment" className="bg-background py-28 md:py-36">
      <div className="mx-auto max-w-[1400px] px-5 md:px-10">
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-4">
          <DemoStep n={1} title="Rohkaffee auswählen" active={!bean}>
            <div className="grid gap-2">
              {beans.map((b) => (
                <button key={b.id} onClick={() => setBean(b.id)} className={`text-left rounded-xl border p-3 ${bean === b.id ? "border-amber bg-amber/10" : "border-border hover:border-foreground/40"}`}>
          ...
          <DemoStep n={2} title="Röstgrad wählen" active={!!bean && !roast}>
            <div className="grid gap-2">
              {roasts.map((rr) => (
                <button key={rr.id} disabled={!bean} onClick={() => setRoast(rr.id)} className={`text-left rounded-xl border p-3 disabled:opacity-40 ${roast === rr.id ? "border-amber bg-amber/10" : "border-border hover:border-foreground/40"}`}>
          ...
          <DemoStep n={3} title="Profil starten" active={!!roast}>
            <button disabled={!roast} onClick={() => setStarted(true)} className="w-full rounded-xl bg-amber text-espresso py-3 disabled:opacity-40 font-medium">
              Röstung starten →
            </button>
            {started && result && r && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-4 rounded-xl border border-border bg-card p-4">
                ...
                <div className="mt-1 font-display text-xl">{result.label} · {r.label}</div>
        ...
        <div className="mt-24">
          ...
          <h3 className="mt-4 font-display tracking-display text-4xl md:text-5xl">Zwei Heimröster. Ein Versprechen.</h3>
                ...
                <article className="rounded-3xl border border-border bg-card overflow-hidden group">
                  ...
                  <div className="p-8">
                    ...
                    <dl className="mt-5 grid grid-cols-2 gap-y-2 text-sm">
                      <dt className="text-muted-foreground">Kapazität</dt><dd>{m.capacity}</dd>
                      <dt className="text-muted-foreground">Röstzeit</dt><dd>{m.time}</dd>
                    </dl>
                    <Link to="/shop" className="mt-6 inline-flex rounded-full bg-foreground text-background px-5 py-2.5 text-sm">Details ansehen →</Link>
                  </div>
                </article>
              </Reveal>
            ))}
...
function DemoStep({ n, title, children, active }: { n: number; title: string; children: React.ReactNode; active: boolean }) {
  return (
    <div className={`rounded-3xl border p-6 transition-colors ${active ? "border-amber bg-amber/5" : "border-border bg-card"}`}>
      <div className="flex items-center gap-3 mb-4">
        <span className={`grid h-7 w-7 place-items-center rounded-full text-xs ${active ? "bg-amber text-espresso" : "bg-muted text-muted-foreground"}`}>{n}</span>
...
function FAQ() {
  const items = [
    ["Ist Heimrösten schwer?", "Nein. Mit vorprogrammierten Profilen läuft die erste Röstung praktisch von allein. Wer mehr will, kann jeden Parameter selbst steuern."],
    ["Wie lange dauert eine Röstung?", "Je nach Profil zwischen 10 und 16 Minuten. Inklusive Abkühlphase planst du etwa 25 Minuten ein."],
    ["Riecht das Rösten stark?", "Es duftet — angenehm und ähnlich wie frisch gebackenes Brot. Unsere Röster haben Filter und Abluft, sodass nichts überhand nimmt."],
    ["Welchen Rohkaffee brauche ich?", "Unverarbeiteten, grünen Kaffee. Du bekommst kuratierte Lots direkt bei uns — sortenrein oder als Probier-Set."],
    ["Für wen lohnt sich ein Heimröster?", "Für alle, die Frische direkt erleben, mehr Kontrolle wollen oder einfach Lust auf ein eigenes Handwerk in der Küche haben."],
  ];
  ...
  return (
    <section className="bg-cream-warm py-28 md:py-36 border-t border-border">
      <div className="mx-auto max-w-[1100px] px-5 md:px-10">
        ...
        <h2 className="mt-4 font-display tracking-display text-5xl md:text-6xl leading-[1]">Was du vielleicht noch wissen willst.</h2>
        <ul className="mt-12 divide-y divide-border border-y border-border">
          {items.map(([q, a], i) => (
            <li key={q}>
              <button onClick={() => setOpen(open === i ? -1 : i)} className="w-full text-left py-6 flex items-start gap-6">
      ...
      <div className="mx-auto max-w-[900px] px-5 md:px-10">
        <Reveal>
          <h2 className="font-display tracking-display text-5xl md:text-7xl leading-[1]">
            Bereit für deine<br /><span className="italic text-amber font-light">erste eigene Röstung?</span>
          </h2>
          <div className="mt-10 flex flex-wrap justify-center gap-3">
            <Link to="/shop" className="rounded-full bg-amber px-6 py-3.5 text-sm font-medium text-espresso">Heimröster kaufen</Link>
            <Link to="/kaffee" className="rounded-full border border-cream/30 px-6 py-3.5 text-sm">Rohkaffee ansehen</Link>
          </div>
        </Reveal>
      </div>
    </section>
