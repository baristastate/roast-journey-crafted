...
export const Route = createFileRoute("/roestereien")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Mehr Sichtbarkeit für Röstereien. Mehr Auswahl für Kaffeeliebhaber. Werde Partner von Barista State." },
      { property: "og:title", content: "Röstereien — Barista State" },
      { property: "og:description", content: "Wir erzählen deine Geschichte und bringen dich zu Menschen, die besonderen Kaffee suchen." },
    ],
  }),
  component: Page,
});
    ...
    <>
      <section className="relative min-h-[80svh] flex items-end theme-dark bg-espresso text-cream overflow-hidden pt-24">
        ...
        <div className="mx-auto max-w-[1400px] px-5 md:px-10 pb-16 md:pb-24 w-full">
          ...
          <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }} className="mt-5 font-display tracking-display text-5xl md:text-7xl lg:text-[5.5rem] leading-[0.95] max-w-5xl">
            Mehr Sichtbarkeit für Röstereien.<br /><span className="italic text-amber font-light">Mehr Auswahl</span> für Kaffeeliebhaber.
          </motion.h1>
          <p className="mt-6 max-w-2xl text-cream/75 text-lg">
            Barista State ist ein kuratierter zusätzlicher Kanal für Röstereien, die ihre Kaffees sichtbarer machen wollen.
          </p>
        </div>
      </section>
          ...
          <div className="grid lg:grid-cols-12 gap-12 items-start">
            <div className="lg:col-span-5">
              ...
              <h2 className="mt-4 font-display tracking-display text-4xl md:text-5xl leading-[1]">
                Wir ersetzen nicht<br />deinen Direktvertrieb.
              </h2>
              <p className="mt-5 text-muted-foreground">
                Wir erzählen deine Geschichte, listen deine Kaffees und bringen dich zu Menschen, die besonderen Kaffee suchen. Du behältst Preis, Marke und Kontrolle.
              </p>
            </div>
      ...
      <section className="bg-cream-warm py-24 md:py-32 border-t border-border">
        <div className="mx-auto max-w-[1400px] px-5 md:px-10">
          ...
          <h2 className="mt-4 font-display tracking-display text-4xl md:text-5xl">Vier Häuser. Vier Handschriften.</h2>
          <div className="mt-12 grid md:grid-cols-2 gap-6">
            {ROASTERIES.map((r) => (
              <Reveal key={r.name}>
                <article className="group relative rounded-3xl overflow-hidden bg-card border border-border">
                  <div className="aspect-[16/10] overflow-hidden">
                    <img src={r.image} alt={r.name} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" />
                  </div>
                  <div className="p-7">
                    <div className="flex items-start justify-between">
                      <div>
                        <h3 className="font-display text-2xl">{r.name}</h3>
                        <div className="text-sm text-muted-foreground">{r.city} · {r.region}</div>
                      </div>
                      <span className="grid h-10 w-10 place-items-center rounded-full border border-border font-display">{r.name[0]}</span>
                    </div>
                    <p className="mt-3 text-sm text-muted-foreground">{r.intro}</p>
                    <div className="mt-4 flex flex-wrap gap-1.5">
                      {r.aromas.map((a) => <span key={a} className="text-xs rounded-full bg-amber/15 px-2.5 py-1">{a}</span>)}
                    </div>
                  </div>
                </article>
              </Reveal>
        ...
        <div className="mx-auto max-w-[1100px] px-5 md:px-10 grid lg:grid-cols-2 gap-12 items-start">
          <Reveal>
            ...
            <h2 className="mt-4 font-display tracking-display text-5xl md:text-6xl leading-[1]">
              Lass uns über<br /><span className="italic text-amber font-light">deinen Kaffee</span> sprechen.
            </h2>
            <p className="mt-5 text-cream/70 max-w-md">
              Kurzes Formular, schnelle Antwort. Wir melden uns in der Regel innerhalb von zwei Werktagen.
            </p>
          </Reveal>
          <Reveal delay={0.1}>
            <form className="space-y-4 rounded-3xl border border-cream/15 p-7 bg-espresso-soft" onSubmit={(e) => e.preventDefault()}>
              {[
                ...
                <label key={l} className="block">
                  ...
                  <input className="mt-1 w-full bg-transparent border-b border-cream/20 py-2 focus:outline-none focus:border-amber" placeholder={p} />
                </label>
              ))}
              <label className="block">
                <span className="text-xs uppercase tracking-[0.2em] text-cream/60">Über euch</span>
                <textarea rows={3} className="mt-1 w-full bg-transparent border-b border-cream/20 py-2 focus:outline-none focus:border-amber" placeholder="Stil, Lots, was euch wichtig ist." />
              </label>
              <button type="submit" className="mt-4 rounded-full bg-amber px-6 py-3 text-sm font-medium text-espresso">Anfrage senden →</button>
            </form>
          </Reveal>
        </div>
      </section>
